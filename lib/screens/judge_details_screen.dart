...
class _JudgeDetailsScreenState extends State<JudgeDetailsScreen> {
  ...
  final List<String> _courtTypes = ['Supreme Court', 'High Court', 'District Court'];
  ...
  final List<String> _highCourtsList = [
    ...
    'Himachal Pradesh High Court', 'High Court of Jammu & Kashmir and Ladakh', 'Jharkhand High Court',
    ...
    'Punjab and Haryana High Court', 'Rajasthan High Court', 'Sikkim High Court', 'Telangana High Court',
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              Text("Specify Your Jurisdiction", style: Theme.of(context).textTheme.headlineSmall, textAlign: TextAlign.center),
              const SizedBox(height: 40),
              
              TextFormField(
                decoration: const InputDecoration(labelText: 'Judicial Service ID'),
                validator: (value) => value == null || value.isEmpty ? 'Your Service ID is required' : null,
              ),
              ...
              DropdownButtonFormField<String>(
                ...
                items: _courtTypes.map((type) => DropdownMenuItem<String>(value: type, child: Text(type))).toList(),
                onChanged: (newValue) {
                  setState(() {
                ...
                validator: (value) => value == null ? 'Please select a court type' : null,
              ),
              const SizedBox(height: 24),
              ...
              if (_selectedCourtType == 'High Court')
                DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(labelText: 'Select High Court'),
                  items: _highCourtsList.map((court) => DropdownMenuItem<String>(value: court, child: Text(court))).toList(),
                  onChanged: (newValue) => setState(() => _selectedHighCourt = newValue),
                  validator: (value) => value == null ? 'Please select a High Court' : null,
                ),
                ...
                Column(
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(labelText: 'State / Union Territory'),
                      items: _indianStatesAndDistricts.keys.map((state) => DropdownMenuItem<String>(value: state, child: Text(state))).toList(),
                      onChanged: (newValue) {
                        setState(() {
                          _selectedState = newValue;
                          _selectedDistrict = null; // Reset district selection
                          _currentDistricts = _indianStatesAndDistricts[newValue] ?? [];
                        });
                      },
                    ...
                    if (_selectedState != null)
                      DropdownButtonFormField<String>(
                        ...
                        items: _currentDistricts.map((district) => DropdownMenuItem<String>(value: district, child: Text(district))).toList(),
  ...
  void _finishSetup() {
    if (_formKey.currentState!.validate()) {
      print('--- Judge Details Captured ---');
      print('Court Type: $_selectedCourtType');
      if (_selectedCourtType == 'High Court') print('Selected High Court: $_selectedHighCourt');
      if (_selectedCourtType == 'District Court') {
      ...
      Navigator.of(context).pushNamedAndRemoveUntil('/judge-dashboard', (route) => false);
    }
  }
}